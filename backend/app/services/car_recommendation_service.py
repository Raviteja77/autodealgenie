...
class CarRecommendationService:
    ...
    async def search_and_recommend(
        ...
        if cached_result:
            ...
            # TODO: Re-enable search history logging with database session dependency injection
    ...
    async def _get_llm_recommendations(
        ...
        try:
            ...
            if hasattr(response, "recommendations") and response.recommendations is not None:
                ...
                # TODO: Re-enable AI response logging with database session dependency injection
                ...
                for vehicle in response.top_vehicles:
                    vehicle_data = next(
                        (item for item in listings if item.get("vin") == vehicle.vin),
                        None,
                    )
                    if vehicle_data:
