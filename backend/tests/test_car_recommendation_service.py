...
async def test_get_cached_result():
    ...
    with patch("app.services.car_recommendation_service.redis_client.get_client", return_value=mock_redis):
...
async def test_search_with_cache_hit():
    ...
    with patch("app.services.car_recommendation_service.redis_client.get_client", return_value=mock_redis):
        with patch("app.repositories.search_history_repository.mongodb.get_collection", return_value=mock_collection):
...
async def test_search_with_retry_logic():
    ...
    with patch("app.services.car_recommendation_service.redis_client.get_client", return_value=mock_redis):
        with patch("app.repositories.search_history_repository.mongodb.get_collection", return_value=mock_collection):
            with patch("app.tools.marketcheck_client.marketcheck_client.search_cars", side_effect=mock_search_cars):
                result = await service.search_and_recommend(
                    make="Toyota", model="RAV4", user_id=1
                )
...
async def test_search_history_logging():
    ...
    with patch("app.services.car_recommendation_service.redis_client.get_client", return_value=mock_redis):
        with patch("app.repositories.search_history_repository.mongodb.get_collection", return_value=mock_collection):
            with patch("app.tools.marketcheck_client.marketcheck_client.search_cars", return_value=mock_api_response):
                with patch("app.tools.marketcheck_client.marketcheck_client.parse_listing", return_value=mock_api_response["listings"][0]):
                    result = await service.search_and_recommend(
                        make="Toyota", model="RAV4", user_id=1
                    )
...
async def test_webhook_triggering():
    ...
    with patch("app.services.car_recommendation_service.redis_client.get_client", return_value=mock_redis):
        with patch("app.repositories.search_history_repository.mongodb.get_collection", return_value=mock_collection):
            with patch("app.tools.marketcheck_client.marketcheck_client.search_cars", return_value=mock_api_response):
                with patch("app.tools.marketcheck_client.marketcheck_client.parse_listing", return_value=parsed_vehicle):
                    with patch("app.services.car_recommendation_service.webhook_service.send_vehicle_alerts", return_value={"total": 1, "success": 1, "failed": 0}) as mock_webhook:
                        result = await service.search_and_recommend(
